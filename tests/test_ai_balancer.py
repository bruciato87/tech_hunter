...
@pytest.mark.asyncio
async def test_normalize_uses_openrouter_when_available(monkeypatch: pytest.MonkeyPatch) -> None:
    balancer = SmartAIBalancer(gemini_keys=["g1"], openrouter_keys=["o1"])

    async def fake_openrouter(api_key: str, prompt: str, title: str) -> tuple[str, str | None]:
        assert api_key == "o1"
        return '"Apple iPhone 14 Pro 128GB colore nero"', "perplexity/sonar"
...
@pytest.mark.asyncio
async def test_normalize_fallbacks_to_heuristic_when_openrouter_fails(monkeypatch: pytest.MonkeyPatch) -> None:
    balancer = SmartAIBalancer(gemini_keys=["g1"], openrouter_keys=["o1"])

    async def fail_openrouter(api_key: str, prompt: str, title: str, model: str | None = None) -> tuple[str, str | None]:
        assert api_key == "o1"
